...
lambda_vec = seq(from = 0.0, to = 200, len = 30)
...
set.seed(1)
  ...
  for(b in 1:B){
    ...
    phat_01 = sweep(p_mat, MARGIN = c(1,2,4), apply(p_mat, c(1,2,4), sum), FUN = "/") # Y1: mis, Y2: obs
    phat_10 = sweep(p_mat, MARGIN = c(1,2,3), apply(p_mat, c(1,2,3), sum), FUN = "/") # Y1: obs, Y2: mis
    phat_00 = sweep(p_mat, MARGIN = c(1,2), apply(p_mat, c(1,2), sum), FUN = "/") # Y1: mis, Y2: mis
